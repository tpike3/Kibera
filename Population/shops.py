...
class Shops(object): 
    ...
    def make_prices(self, store_type): 
        ...
        if store_type == "24_cooked" or store_type == "cooked" or store_type ==  "job_location":
            food_prices["pack_2"] = np.random.normal(9.1, 1) # for rice and oil, 
                                                                       # (millet or wheat), + 1.5 ksh for prep
            food_prices["pack_3"] = np.random.normal(13, 1.5) #for potatoes, maize flours,
                                                                         # and oil +1.5 ksh for prep
                                                                       
            food_prices["pack_4"] = np.random.normal(32, 1.5) #for beans, maize flours, kale
                                                                         # and oil +1.5 ksh for prep
                                                                         
            
        if store_type == "24_raw" or store_type == "raw":
            food_prices["pack_2"] = np.random.normal(7.6, 1) # for rice and oil
            food_prices["pack_3"] = np.random.normal(11.5, 1.5) # for rice, oil, and flour 
                                                                       # (millet or wheat)
            food_prices["pack_4"] = np.random.normal(30.5, 1.5) #for oil, maize flours, beans, kale
            food_prices["fuel"] =   np.random.normal(2, 1) #for kerosene                                                           # and oil
